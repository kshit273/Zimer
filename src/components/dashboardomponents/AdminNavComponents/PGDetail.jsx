...
const StarRow = ({ label, value }) => {
  ...
  return (
    <div className="flex items-center justify-between">
      <span className="text-[0.6rem] tracking-[0.12em] text-[#555550] uppercase">{label}</span>
      <div className="flex items-center gap-1.5">
        <div className="flex gap-0.5">
          {[1, 2, 3, 4, 5].map((s) => (
            <span
              key={s}
              className={`text-[10px] ${s <= filled ? "text-[#c8f135]" : "text-[#2a2a2a]"}`}
            >
              ★
            </span>
          ))}
        </div>
        <span className="text-[0.62rem] text-[#e8e8e0]">{value ? value.toFixed(1) : "—"}</span>
      </div>
    </div>
  );
};
...
const Tag = ({ text, accent = false }) => (
  <span
    className={`inline-block px-2 py-0.5 rounded-md text-[0.6rem] tracking-[0.1em] uppercase border
      ${accent
        ? "border-[#c8f135]/30 text-[#c8f135] bg-[#c8f135]/5"
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-[0.6rem] text-[#555550] mt-0.5">
            ...
            {pendingCount > 0 && (
              <span className="ml-2 text-[#c8f135]">· {pendingCount} pending rent</span>
            )}
          </p>
        </div>
              ...
              <div className="flex flex-col gap-2">
                {room.tenants.map((t, i) => {
                  ...
                  return (
                    <div
                      key={i}
                      className="flex items-center gap-3 px-3 py-2.5 bg-[#111111] border border-[#1f1f1f] rounded-lg"
                    >
                      {/* Avatar initial */}
                      <div className="flex-shrink-0 w-7 h-7 rounded-md bg-[#1a1a1a] border border-[#2a2a2a] flex items-center justify-center">
                        <span className="text-[0.6rem] text-[#555550]">T</span>
                      </div>

                      <div className="flex-1 min-w-0">
                        <p className="text-[0.65rem] text-[#e8e8e0] font-medium truncate">
  ...
  return (
    <div className="font-dm-mono flex flex-col h-full">
      ...
      <div className="flex items-center justify-between mb-5 pb-4 border-b border-[#1f1f1f]">
        ...
        <span className="text-[0.55rem] tracking-[0.15em] text-[#2a2a2a] uppercase">{pg.RID}</span>
      </div>
      ...
      <div className="flex-1 overflow-y-auto pr-1 scrollbar-hide flex flex-col gap-0">
        ...
        <div className="flex gap-5 mb-6">
          {/* Cover photo */}
          <div className="flex-shrink-0 w-[180px] h-[140px] rounded-xl overflow-hidden bg-[#1a1a1a] border border-[#1f1f1f]">
            {pg.coverPhoto ? (
              ...
              >
                <span className="text-[#2a2a2a] text-[0.55rem] tracking-[0.15em] uppercase">No photo</span>
              </div>
            )}
          </div>
          ...
          <div className="flex flex-col justify-between py-0.5 flex-1 min-w-0">
            <div>
              ...
              <p className="text-[0.68rem] text-[#555550] leading-relaxed line-clamp-2">{pg.address}</p>
        ...
        {pg.description && (
          <>
            ...
            <p className="text-[0.72rem] text-[#888] leading-relaxed mb-2">{pg.description}</p>
            {pg.additionalInfo && (
              <p className="text-[0.68rem] text-[#555550] leading-relaxed mt-1">{pg.additionalInfo}</p>
            )}
            <Divider />
          </>
        )}
            ...
            <ul className="flex flex-col gap-1.5 mb-2">
              {pg.rules.map((r, i) => (
                <li key={i} className="flex items-start gap-2">
                  <span className="text-[#c8f135] text-[0.55rem] mt-0.5">▸</span>
                  <span className="text-[0.68rem] text-[#888] leading-relaxed">{r}</span>
                </li>
              ))}
            </ul>
            <Divider />
        ...
        {(pg.foodAvailable || pg.tiffinServiceAvailable) && (
          <>
            ...
            <div className="flex flex-col gap-1.5 mb-2">
              {pg.foodDescription && (
                <p className="text-[0.7rem] text-[#888] leading-relaxed">{pg.foodDescription}</p>
              )}
              {pg.menu?.length > 0 && (
...
const QuickStat = ({ label, value, accent = false }) => (
  <div className="flex flex-col gap-0.5">
    <span className="text-[0.52rem] tracking-[0.14em] text-[#555550] uppercase">{label}</span>
