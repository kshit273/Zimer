...
exports.postReport = async (req, res) => {
  try {
    ...
    const existingReport = await AdminNotification.findOne({
      sender: senderId,
      pg: pgId,
    ...
    if (existingReport) {
      return res.status(400).json({
        success: false,
        message: "At max 1 report request can be sent within 24 hours",
      });
    }

    // Fetch tenant
    const tenant = await Tenant.findById(senderId).select(
    ...
    // Create message
    const message = `A report has been generated by ${tenant.firstName} ${tenant.lastName} (tenantId: ${senderId}) of PG ${pgId}`;
